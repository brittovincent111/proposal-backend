...
async function seed(): Promise<void> {
  ...
  try {
    ...
    await documents.addPackage(organizationId, documentId, userId, {
      packageId: created._id.toString(),
    });
    const revision = await documents.generate(organizationId, documentId, userId);

    logger.log(`Generated ${document.documentNumber}`);
    logger.log('');
